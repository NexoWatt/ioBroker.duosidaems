...
class LocalClient {
  ...
  _acceptParsedStatus(parsed) {
    if (parsed && parsed.deviceId && !this.deviceId) {
      this.deviceId = parsed.deviceId;
    }
    return parsed;
  }

  async _readStatusOnce(maxFrames = 5) {
    return this._withSession(async session => {
      await session.write(HANDSHAKE_1);
      await sleep(100);
      ...
      try {
        const firstChunk = await session.nextChunk(1000);
        const firstParsed = this._parseStatusFrame(firstChunk);
        if (firstParsed) {
          return this._acceptParsedStatus(firstParsed);
        }
      } catch {
        // Some devices do not answer the first handshake frame.
      }
      ...
      const queuedChunks = session.drainQueue();
      for (const queuedChunk of queuedChunks) {
        const queuedParsed = this._parseStatusFrame(queuedChunk);
        if (queuedParsed) {
          return this._acceptParsedStatus(queuedParsed);
        }
      }
      ...
      while (Date.now() < deadline && framesLeft > 0) {
        ...
        if (parsed) {
          return this._acceptParsedStatus(parsed);
        }
        framesLeft -= 1;
      }
  ...
  async readStatus(maxFrames = 5, maxAttempts = 3) {
    const attempts = Math.max(1, Number(maxAttempts) || 1);
    let lastError = null;

    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        return await this._readStatusOnce(maxFrames);
      } catch (error) {
        lastError = error;
        if (!this._isRetryableReadError(error) || attempt >= attempts) {
          throw error;
        }
        if (this.logger && typeof this.logger.debug === 'function') {
          this.logger.debug(`Local status read attempt ${attempt} failed (${error.message}). Retrying.`);
        }
        await sleep(150);
      }
    }

    throw lastError || new Error('No local status frame received');
  }
